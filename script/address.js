$(document).on(`change`, `.addressDivision`, function (e) {
    ...
    $(`#${orderId}addressDistrict`).html(`<option value="0" selected>Select One</option>`);
    if (id != `0`) {
        $.map(jsonSearch(`districts`, `division_id`, id), function (district, indexOrKey) {
...
$(document).on(`change`, `.addressDistrict`, function (e) {
    var id = $(this).val();
    var orderId = $(this).attr('orderId');
    $(`#${orderId}addressUpazila`).html(`<option value="0" selected>Select One</option>`);
    if (id != `0`) {
        $.map(jsonSearch(`upazilas`, `district_id`, id), function (district, indexOrKey) {
...
function getAddress(orderId) {
    ...
    if (address.division == `0` || address.upazila == `0` || address.address == `` || address.district == `0`) {
        return null;
    }
    return address;
}
...
function showAddressProfile(orderId, address) {
    ...
    $.map(jsonSearch(`districts`, `division_id`, `${address.division}`), function (district, indexOrKey) {
        $(`#${orderId}addressDistrict`).append(`
            <option value="${district.id}">${district.name}</option>
        `);
    });
    $(`#${orderId}addressDistrict`).val(address.district);
    $.map(jsonSearch(`upazilas`, `district_id`, `${address.district}`), function (district, indexOrKey) {
        $(`#${orderId}addressUpazila`).append(`
                <option value="${district.id}">${district.name}</option>
                `);
    });
...
function showAddress(address) {
    console.log();
    return `
    ${address.address} , ${jsonSearchId('upazilas', address.upazila.toString())[0].name} ,
    ...
    if (address) {
        $.ajax({
            ...
            success: function (response) {
                ...
                $(`#${orderId}addressRow`).html(`<hr><p><strong>Deliver Address : </strong> ${showAddress(response.order.address)} </p>`);
            }
        });
    }
