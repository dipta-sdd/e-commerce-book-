...
<script>
    $(document).ready(function () {
        sidebarActivate('./processing_order');
        let userRole = $('#userType').attr('usertype');
        if(userRole == 'admin'){
            $.ajax({
                type: "POST",
                url: "./api/v1/admin/findOrder",
                data:JSON.stringify({
                    filter :'Processing'
                }),
                    ...
                    $.map(orders, function (order, indexOrKey) {
                        ...
                        $(`#row${order._id}`).append(`
                            ...
                            <div class="col-3 center">
                                <button type="button" class="ms-auto btn btn-outline-primary readyToship" orderId="${order._id}"><i class="fa-solid fa-truck-fast"></i>Ready for shipping</button>
                            </div>
                        ` );
                    });
                }
        ...
        $(document).on('click','.readyToship', function(e){
            e.preventDefault();
            var orderId = $(this).attr('orderId');
